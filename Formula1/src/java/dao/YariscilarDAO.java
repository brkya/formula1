...
public class YariscilarDAO {
    ...
    private TakimlarDAO takimDAO;
        ...
        try {
            PreparedStatement st = this.getConnection().prepareStatement("select * from yariscilar where yarisci_id=?");
            st.setLong(1, id);
    ...
    public List<Yariscilar> listele() {
        ...
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("select * from yariscilar");
            ...
            while (rs.next()) {
                ...
                tmp.setTakim(this.takimDAO.idBul(rs.getLong("takim_id")));
                yarisciList.add(tmp);
    ...
    public void ekle(Yariscilar yarisci) {
        try {
            ...
            PreparedStatement pst = this.getConnection().prepareStatement("insert into pistler (yarisci_adi,yarisci_soyadi,yas,dogum_yeri,puan,takim_id) values(?,?,?,?,?,?)");
    ...
    public void sil(Yariscilar yarisci) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("delete from yariscilar where yarisci_id=?");
    ...
    public void duzelt(Yariscilar yarisci) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("update yariscilar set yarisci_adi=?,yarisci_soyadi=?,yas=?,dogum_yeri=?,puan=?,takim_id=? where yarisci_id =?");
            pst.setString(1, yarisci.getYarisci_adi());
            pst.setString(2, yarisci.getYarisci_soyadi());
            pst.setInt(3, yarisci.getYas());
            pst.setString(4, yarisci.getDogum_yeri());
        ...
        try {
            PreparedStatement st = this.getConnection().prepareStatement("select * from yaris_yarisci where yaris_id=?");
