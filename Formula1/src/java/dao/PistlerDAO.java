...
public class PistlerDAO {
    ...
    public Pistler idBul(Long id) {
        ...
        try {
            PreparedStatement st = this.getConnection().prepareStatement("select * from pistler where pist_id=?");
            st.setLong(1, id);
    ...
    public void ekle(Pistler pist) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("insert into pistler (pist_adi,tur_sayisi,ulke) values(?,?,?)");
