...
public class YarisDAO {
    ...
    public Yaris idBul(Long id) {
        ...
        try {
            PreparedStatement st = this.getConnection().prepareStatement("select * from yaris where yaris_id=?");
            st.setLong(1, id);
    ...
    public List<Yaris> listele() {
        ...
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("select * from yaris");
    ...
    public void ekle(Yaris yaris) {
        try {
            ...
            PreparedStatement pst = this.getConnection().prepareStatement("insert into yaris (yaris_adi,tarih,pist_id) values(?,?,?)");
    ...
    public void sil(Yaris yaris) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("delete from yaris where yaris_id=?");
    ...
    public void duzelt(Yaris yaris) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("update yarislar set yaris_adi=?,tarih=?,pist_id=? where yaris_id=?");
            pst.setString(1, yaris.getYaris_adi());
            pst.setDate(2, (Date) yaris.getTarih());
            pst.setLong(3, yaris.getPist().getPist_id());
