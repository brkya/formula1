...
public class TakimKoclariDAO {
    ...
    public TakimKoclari idBul(Long id) {
        ...
        try {
            PreparedStatement st = this.getConnection().prepareStatement("select * from takim_koclari where koc_id=?");
            st.setLong(1, id);
    ...
    public List<TakimKoclari> listele() {
        ...
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("select * from takim_koclari");
    ...
    public void ekle(TakimKoclari koc) {
        try {
            ...
            PreparedStatement pst = this.getConnection().prepareStatement("insert into takim_koclari (koc_adi,koc_soyadi,yas,dogum_yeri) values(?,?,?,?)");
    ...
    public void sil(TakimKoclari koc) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("delete from takim_koclari where koc_id=?");
    ...
    public void duzelt(TakimKoclari koc) {
        try {
            PreparedStatement pst = this.getConnection().prepareStatement("update takim_koclari set koc_adi=?,koc_soyadi=?,yas=?,dogum_yeri=? where koc_id=?");
            pst.setString(1, koc.getKoc_adi());
            pst.setString(2, koc.getKoc_soyadi());
            pst.setInt(3, koc.getYas());
            pst.setString(4, koc.getDogum_yeri());
